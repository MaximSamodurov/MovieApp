...
class FullInfoMovieCell: UICollectionViewCell, UICollectionViewDelegateFlowLayout {
    ...
    let addToFavorites = UILabel(text: "Add To Favorites", font: .boldSystemFont(ofSize: 20))
    ...
    override init(frame: CGRect) {
        ...
        let addFavorites = UIStackView(arrangedSubviews: [addToFavorites, likeButton, UIView()])
        addFavorites.spacing = 6
        ...
        let textStackView = VerticalStackView(arrangedSubViews: [title, addFavorites, description, director, genre, actors, awards, year], spacing: 8)
